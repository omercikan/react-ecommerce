  ...
  const searchedLikes = useMemo(() => {
    return likes.filter((product) =>
      product.title
        .trim()
        .toLowerCase()
        .includes(defferedSearch.trim().toLowerCase())
    );
  }, [likes, defferedSearch]);
      ...
      <div className="mt-[122px]">
        <div className="likes-container">
          <div className="breadcrumbs my-20 flex items-center gap-1 text-[#999999]">
            <Link
              to="/"
              className="flex items-center gap-1 hover:text-[#232323] transition duration-500"
            >
              Home
            </Link>
          ...
          <div className="mb-10 border-b pb-10 text-2xl uppercase flex justify-between items-center">
            ...
            <input
              type="text"
              placeholder="Search in my favorites"
              className="border outline-none text-sm py-2 px-3"
