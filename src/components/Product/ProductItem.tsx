  ...
  return (
    <li className="border rounded-lg hover:shadow-md transition duration-300 p-10 max-sm:w-full mb-5">
      ...
      <div className="product-content">
        <h2 className="whitespace-nowrap text-ellipsis overflow-hidden mt-10 mb-5">
          {title}
        </h2>

        <data value={price} className="text-[#e10600] font-bold">
          ${price}
        </data>

        <Link to={`/product-detail/${title.substring(0, 30)}:${id}`}>
          <button className="block whitespace-nowrap mt-10 border p-2 w-full rounded-3xl hover:bg-black hover:text-white hover:border-black transition duration-500">
            Review Product
          </button>
        </Link>
      </div>
