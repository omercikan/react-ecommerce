...
const ShoppingCart: React.FC = () => {
    ...
    const cartTotal = cart.reduce((acc, item) => acc += (item.productPrice * item.productQuantity), 0).toFixed(2);
    const dispatch = useDispatch<AppDispatch>();

    return (
        <React.Fragment>
            {isOpenCart && (
                <div className='shopping-cart-container fixed top-0 left-0 w-full h-screen'>
                    <div className='shopping-cart-wrapper bg-white w-[420px] h-screen fixed right-0 top-0 max-sm:w-[95%]'>
                        <div className="shopping-cart-wrapper__title py-[24px] px-[30px]">
                            ...
                            <span className='text-[#727272] text-sm mt-2 inline-block'>{cart.length} items</span>
                        </div>
